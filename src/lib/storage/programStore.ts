...
// Programs extracted from uw_undergraduate_degrees.txt
...
/**
 * Parse "Enrolled in" restrictions from prerequisitesHtml
 * Returns array of program IDs that are allowed to take the course
 */
export function parseEnrolledInRestriction(html: string | null | undefined): ProgramId[] {
  if (!html) return [];
  ...
  // Extract program names from the HTML (they appear in anchor tags)
...
/**
 * Check if a course is accessible to the selected program based on HTML restrictions
 * This now uses the actual "Enrolled in" data from the course prerequisites
 */
export function isCourseAccessibleToProgram(
  ...
  // If we have prereqHtml, parse the enrolled restriction
  if (prereqHtml) {
    const allowedPrograms = parseEnrolledInRestriction(prereqHtml);
    
    // If no enrollment restriction found, course is open to all
